<?php
/**
 * cashier_functions.php - Fungsi-fungsi khusus untuk panel cashier
 * 
 * File ini berisi fungsi-fungsi yang dibutuhkan untuk operasi kasir POS Apotek
 * 
 * @version 1.0.0
 * @date 2025-04-17
 */
...
/*function searchProductsForCashier($pdo, $keyword) {
    try {
        $query = "SELECT p.product_id, p.product_name, p.kode_item, 
                         p.price, p.stock_quantity, p.unit, p.requires_prescription,
        ...
        $stmt->bindParam(':barcode', $keyword); // Pencarian barcode harus sama persis
        $stmt->execute();
...
function savePharmacyTransaction($pdo, $data) {
    try {
        ...
        $query = "INSERT INTO sales (
                    invoice_number, customer_name, doctor_id, prescription_number, 
                    user_id, subtotal, tax_amount, discount_amount, 
                    total_amount, payment_method_id, payment_status, notes
                  ) VALUES (
                    :invoice_number, :customer_name, :doctor_id, :prescription_number, 
                    :user_id, :subtotal, :tax_amount, :discount_amount, 
                    :total_amount, :payment_method_id, :payment_status, :notes
        ...
        $stmt->bindParam(':prescription_number', $data['prescription_number']);
        ...
        foreach ($data['items'] as $item) {
            // Menggunakan batch dengan prinsip FIFO (First In First Out)
            // Ambil batch yang belum kadaluarsa dengan tanggal kadaluarsa terdekat
            $quantityLeft = $item['quantity'];
            
            while ($quantityLeft > 0) {
                $query = "SELECT pb.batch_id, pb.remaining_quantity 
                ...
                if (!$batch) {
                    throw new Exception("Tidak ada batch yang tersedia untuk produk ID: " . $item['product_id']);
                }
                
                $batchId = $batch['batch_id'];
                $batchQuantity = min($quantityLeft, $batch['remaining_quantity']);
                
                // Insert sale item untuk batch ini
                ...
                $stmt->bindParam(':quantity', $batchQuantity);
                $stmt->bindParam(':unit_price', $item['unit_price']);
                
                // Gunakan discount_percent jika ada, jika tidak set ke 0
                $discountPercent = isset($item['discount_percent']) ? $item['discount_percent'] : 0;
                $stmt->bindParam(':discount_percent', $discountPercent);
                
                $itemTotal = $batchQuantity * $item['unit_price'] * (1 - $discountPercent/100);
                $stmt->bindParam(':item_total', $itemTotal);
                $stmt->execute();
                
                // Update remaining quantity di batch
                $query = "UPDATE product_batches 
