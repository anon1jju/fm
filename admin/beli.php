 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
     ...
     function unformat_rupiah_php($formatted_value) {
        if (empty($formatted_value) || !is_string($formatted_value)) return 0.0;
        $cleaned = str_replace('.', '', $formatted_value); // Remove thousand separators (e.g., "100000,00")
        $cleaned = str_replace(',', '.', $cleaned);      // Replace decimal comma with dot (e.g., "100000.00")
                 ...
                 try {
                     ...
                     foreach ($product_ids as $key => $product_id_item) { 
                         if (empty($product_id_item)) continue;
                            ...
                            // Get raw formatted strings from POST
                            $unit_price_raw = $unit_prices[$key]; 
                            $sell_price_raw = $sell_prices_form[$key];
                            
                            // Unformat them to numeric values
                            $unit_price_numeric = unformat_rupiah_php($unit_price_raw);
                            $sell_price_numeric = unformat_rupiah_php($sell_price_raw);
                            ...
                            $item_total = $quantity * $unit_price_numeric; // Use numeric value for calculation
                            ...
                            $stmt_item->bindParam(':unit_price', $unit_price_numeric); // Bind the numeric value
                            ...
                            $stmt_update_product->bindParam(':sell_price_form', $sell_price_numeric); // Use numeric value
                            $stmt_update_product->bindParam(':purchase_price_form', $unit_price_numeric); // Use numeric value
                            $stmt_update_product->bindParam(':product_id', $product_id_item, PDO::PARAM_INT);
                            $stmt_update_product->execute();
 
                         $batch_number_item = isset($batch_numbers_form[$key]) && !empty($batch_numbers_form[$key]) ? $batch_numbers_form[$key] : '';
                   ...
                   ON DUPLICATE KEY UPDATE 
                       ...
                       updated_at = NOW()";
                            ...
                            $stmt_prod_batch->bindParam(':purchase_price', $unit_price_numeric); // Use numeric value
                     ...
                     if ($payment_status === 'cicil' && isset($_POST['initial_amount_paid']) && !empty($_POST['initial_amount_paid'])) {
                         ...
                         $initial_amount_paid_val = unformat_rupiah_php($_POST['initial_amount_paid']);
                     ...
                     if (empty($error) || (strpos($error, 'Gagal mencatat cicilan awal') === false && strpos($error, 'Gagal mengunggah bukti cicilan awal') === false && strpos($error, 'File bukti cicilan awal tidak valid') === false && strpos($error, 'Jumlah cicilan awal melebihi total pembelian') === false)) {
                         $pdo->commit();
                         $message = "Pembelian berhasil ditambahkan dengan ID: " . $purchase_id . ". Total: Rp " . number_format($total_purchase_amount, 0, ',', '.') . (empty($message) ? "" : "<br>" . $message);
                         $_POST = array(); 
 ...
 <head>
     ...
     <style>
         ...
         #purchaseItemsSectionWrapper table thead th:first-child {
             width: 350px; /* Naikkan nilai ini, misalnya dari 300px menjadi 350px atau lebih */
         ...
         #purchaseItemsSectionWrapper table tbody td:first-child {
             width: 350px; /* Pastikan lebar ini sama dengan yang di th agar kolomnya lurus */
         }
             ...
             <div class="container-fluid">
                 <div class="grid grid-cols-12 gap-x-6">
                     <div class="xl:col-span-12 col-span-12">
                         <div class="box">
                             <div class="box-header"><div class="box-title"><span><a href="hutang_supplier.php" class="ti-btn ti-btn-sm ti-btn-info"><i class="ri-arrow-left-s-line"></i>Kembali</a></span> <span class="ms-2">Form Input Pembelian</span></div></div>
                             <div class="box-body p-6">
                                 ...
                                 <form method="POST" action="beli.php" id="purchaseForm" enctype="multipart/form-data">
                                     <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 items-end">
                                         <div> <!-- Kontainer luar untuk label dan grup input-tombol -->
                                             ...
                                             <div class="relative"> <!-- Div relative untuk input dan icon container -->
                                                 <input type="text" class="ti-form-input pr-10 w-full" id="invoice_number_supplier" name="invoice_number_supplier" value="<?php echo isset($_POST['invoice_number_supplier']) ? htmlspecialchars($_POST['invoice_number_supplier']) : ''; ?>" required>
                                                 ...
                                                 <div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none"> 
                                                     ...
                                                     <button type="button" onclick="generateInvoiceNumber()" class="text-gray-500 hover:text-blue-500 focus:outline-none pointer-events-auto"> <!-- Pastikan ini bisa di-klik -->
                                     ...
                                     <div class="flex justify-between items-center mb-4">
                                         ...
                                         <a href="hutang_supplier.php" class="ti-btn ti-btn-primary">Laporan Pembelian Supplier</a>
                                     </div> 
                                         ...
                                         <table class="min-w-full divide-y divide-gray-200 border border-gray-200 dark:border-gray-700 rounded-md mb-4">
                                             <thead class="bg-gray-200 dark:bg-gray-800">
                                                 <tr>
                                                     ...
                                                     <th class="px-2 py-3 text-left text-xs font-semibold text-gray-100 dark:text-gray-300 uppercase tracking-wider">Qty <span class="text-red-500">*</span></th>
                                                     <th class="px-2 py-3 text-left text-xs font-semibold text-gray-100 dark:text-gray-300 uppercase tracking-wider">Harga Beli <span class="text-red-500">*</span></th>
                                                     <th class="px-2 py-3 text-left text-xs font-semibold text-gray-100 dark:text-gray-300 uppercase tracking-wider">Harga Jual <span class="text-red-500">*</span></th>
                                                     <th class="px-2 py-3 text-left text-xs font-semibold text-gray-100 dark:text-gray-300 uppercase tracking-wider">Total Item</th>
                                                     <th class="px-2 py-3 text-left text-xs font-semibold text-gray-100 dark:text-gray-300 uppercase tracking-wider">Batch</th>
                                                     <th class="px-2 py-3 text-left text-xs font-semibold text-gray-100 dark:text-gray-300 uppercase tracking-wider">Expire</th>
                                                     <th class="px-2 py-3 text-left text-xs font-semibold text-gray-100 dark:text-gray-300 uppercase tracking-wider">Aksi</th>
                                                 </tr>
                                             </thead>
                                             <tbody id="itemsContainer" class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                                                 ...
                                                 <tr class="item-row">
                                                     <td class="px-4 py-2 whitespace-nowrap product-search-input-container">
                                                        <input type="text"
                                                            class="ti-form-input product-search-input text-lg font-semibold"
                                                            name="product_search_display[]"
                                                            placeholder="Cari Produk/Kode/Barcode..."
                                                            style="width: 100%; max-width: 300px; padding: 10px; font-size: 16px;"
                                                            autocomplete="off"
                                                            value="<?php echo isset($_POST['product_search_display'][$i]) ? htmlspecialchars($_POST['product_search_display'][$i]) : ''; ?>">
                                                        <input type="hidden" name="product_id[]" class="actual-product-id"
                                                            value="<?php echo isset($_POST['product_id'][$i]) ? htmlspecialchars($_POST['product_id'][$i]) : ''; ?>">
                                                        <div class="product-search-results"></div>
                                                    </td>

                                                     <td class="px-2 py-2 whitespace-nowrap"><input type="tel" name="quantity[]" class="ti-form-input quantity-input text-center" placeholder="Qty" required value="<?php echo isset($_POST['quantity'][$i]) ? htmlspecialchars($_POST['quantity'][$i]) : '1'; ?>" style="width: 60px;"></td>
                                                     <td class="px-2 py-2 whitespace-nowrap"><input type="tel" step="any" name="purchase_price[]" class="ti-form-input purchase-price-input" style="width: 100px" min="0" value="<?php echo isset($_POST['purchase_price'][$i]) ? htmlspecialchars($_POST['purchase_price'][$i]) : ''; ?>" required placeholder="Harga Beli"></td>
                                                     <td class="px-2 py-2 whitespace-nowrap"><input type="tel" step="any" name="sell_price[]" class="ti-form-input sell-price-input" style="width: 100px" min="0" value="<?php echo isset($_POST['sell_price'][$i]) ? htmlspecialchars($_POST['sell_price'][$i]) : ''; ?>" required placeholder="Harga Jual"></td>
                                                     <td class="px-2 py-2 whitespace-nowrap"><input type="text" class="ti-form-input item-total-display" readonly placeholder="Rp 0" style="background-color: #e9ecef; width: 120px;"></td>
                                                     <td class="px-2 py-2 whitespace-nowrap"><input type="text" name="batch_number[]" class="ti-form-input batch-number-input" placeholder="Batch" style="width: 100px;" value="<?php echo isset($_POST['batch_number'][$i]) ? htmlspecialchars($_POST['batch_number'][$i]) : ''; ?>"></td>
                                                     <td class="px-2 py-2 whitespace-nowrap"><input type="text" name="expiry_date[]" class="ti-form-input expiry-date-input" style="width: 120px;" value="<?php echo isset($_POST['expiry_date'][$i]) ? htmlspecialchars($_POST['expiry_date'][$i]) : ''; ?>"></td>
                                                     <td class="px-2 py-2 whitespace-nowrap"><button type="button" class="ti-btn ti-btn-danger ti-btn-icon removeItemBtn"><i class="ri-delete-bin-line"></i></button></td>
                                                 </tr>
                                                 <?php endfor; ?>
                                             </tbody>
                                         </table>
                                             ...
                                             <div class="space-y-4">
                                                 ...
                                                 <div class="flex flex-col md:flex-row gap-4 items-end pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
                                                     <div class="w-full md:w-1/2">
                                                         ...
                                                         <input type="tel" class="ti-form-input text-center" id="due_days" name="due_days" value="<?php echo isset($_POST['due_days']) ? htmlspecialchars($_POST['due_days']) : '30'; ?>" placeholder="Contoh: 30">
                                                     </div>
                                                     <div class="w-full md:w-1/2">
                                                         ...
                                                         <input type="text" class="ti-form-input flatpickr-date" id="due_date" name="due_date" placeholder="Otomatis/Manual" value="<?php
                                                                 ...
                                                                 if (isset($_POST['due_date']) && !empty($_POST['due_date'])) {
                                                                     ...
                                                                     try { $pd = new DateTime($_POST['purchase_date']); $dd = (int)$_POST['due_days']; if($dd>=0){ $pd->add(new DateInterval("P{$dd}D")); $display_due_date_form = $pd->format('Y-m-d');} } catch (Exception $e) { }
                                                                 } else { try { $pd = new DateTime(date('Y-m-d')); $dd = 30; $pd->add(new DateInterval("P{$dd}D")); $display_due_date_form = $pd->format('Y-m-d'); } catch (Exception $e) { } }
                                             ...
                                             <div id="initial-installment-section" class="space-y-4 hidden">
                                                 <div class="grid grid-cols-1 md:grid-cols-2 gap-4"> <!-- Ini baris utama form dua kolom -->
                                                     ...
                                                     <div>
                                                         ...
                                                         <input type="tel" step="any" class="ti-form-input" id="initial_amount_paid" name="initial_amount_paid" min="0" value="<?php echo isset($_POST['initial_amount_paid']) ? htmlspecialchars($_POST['initial_amount_paid']) : ''; ?>">
 ...
 <script>
     // Fungsi generateInvoiceNumber tetap seperti sebelumnya
     // function generateInvoiceNumber() {
     //     const randomNumber = Math.floor(10000 + Math.random() * 90000);
     //     document.getElementById("invoice_number_supplier").value = randomNumber;
     // }
     // Note: generateInvoiceNumber dipindahkan ke dalam DOMContentLoaded listener di bawah agar bisa akses savePrimaryFormData
 </script>
 
 <script>
     $(document).ready(function () {
         // $(".select2").select2({ placeholder: "Pilih Supplier", allowClear: true });
         // Note: Inisialisasi Select2 dipindahkan ke dalam DOMContentLoaded listener utama
     });
 </script>
 ...
 <script>
     ...
     document.addEventListener('DOMContentLoaded', function () {
         ...
         // Helper function to unformat Rupiah string to number
         function unformatRupiah(formattedValue) {
            if (typeof formattedValue !== 'string' || formattedValue.trim() === '') return 0;
            // Remove thousands separators (.), replace decimal comma (,) with a period (.)
         ...
         function savePrimaryFormData() {
             const data = {
                 ...
                 initial_amount_paid: initialAmountPaidInput.value, // Assuming this is raw number
         ...
         function saveItemRowsData() {
             ...
             itemsContainer.querySelectorAll('.item-row').forEach(row => {
                 ...
                 items.push({
                     ...
                     purchase_price: row.querySelector('.purchase-price-input').value, // Saved as formatted string
                     sell_price: row.querySelector('.sell-price-input').value,         // Saved as formatted string
         ...
         function getDefaultItemRowHTML() {
              return `
                 <td class="px-2 py-2 whitespace-nowrap product-search-input-container">
                 ...
                 <td class="px-2 py-2 whitespace-nowrap"><input type="tel" name="quantity[]" class="ti-form-input quantity-input text-center" placeholder="Qty" required value="1" style="width: 60px;"></td>
                 <td class="px-2 py-2 whitespace-nowrap"><input type="tel" step="any" name="purchase_price[]" class="ti-form-input purchase-price-input" style="width: 100px" min="0" value="" required placeholder="Harga Beli"></td>
                 <td class="px-2 py-2 whitespace-nowrap"><input type="tel" step="any" name="sell_price[]" class="ti-form-input sell-price-input" style="width: 100px" min="0" value="" required placeholder="Harga Jual"></td>
                 <td class="px-2 py-2 whitespace-nowrap"><input type="text" class="ti-form-input item-total-display" readonly placeholder="Rp 0" style="background-color: #e9ecef; width: 120px;"></td>
                 <td class="px-2 py-2 whitespace-nowrap"><input type="text" name="batch_number[]" class="ti-form-input batch-number-input" placeholder="Batch" style="width: 100px;"></td>
                 <td class="px-2 py-2 whitespace-nowrap"><input type="text" name="expiry_date[]" class="ti-form-input expiry-date-input" style="width: 120px;"></td>
                 <td class="px-2 py-2 whitespace-nowrap"><button type="button" class="ti-btn ti-btn-danger ti-btn-icon removeItemBtn"><i class="ri-delete-bin-line"></i></button></td>
             `;
         }
         ...
         function addItemRow(itemData = null) {
             ...
             initializeExpiryDateFlatpickr(newExpiryInput); 
             ...
             if (itemData) {
                 ...
                 // itemData.purchase_price and sell_price are already formatted from localStorage
                 ...
                 if (newExpiryInput && newExpiryInput._flatpickr && itemData.expiry_date) {
                      newExpiryInput._flatpickr.setDate(itemData.expiry_date, false); 
                 }
             }
         ...
         [invoiceNumberSupplierInput, dueDaysInput, notesTextarea, initialAmountPaidInput, initialPaymentMethodInput, initialPaymentReferenceInput]
             .forEach(el => { if(el) el.addEventListener('input', savePrimaryFormData); });
         
         if(supplierIdSelect) $(supplierIdSelect).on('change', savePrimaryFormData); 
         ...
         function updateGrandTotal() {
             ...
             itemsContainer.querySelectorAll('.item-row').forEach(row => {
                 ...
                 const purchasePrice = unformatRupiah(row.querySelector('.purchase-price-input').value); // MODIFIED
         ...
         function updateItemTotal(rowElement) {
             ...
             const purchasePrice = unformatRupiah(rowElement.querySelector('.purchase-price-input').value); // MODIFIED
         ...
         function attachCalculationListeners(rowElement) {
             ...
             // Also re-save if sell price is manually changed, though it doesn't affect total
             rowElement.querySelector('.sell-price-input').addEventListener('input', saveItemRowsData);
         }
         ...
         function initializeProductSearch(rowElement) {
             ...
             const purchasePriceInput = rowElement.querySelector('input[name="purchase_price[]"]'); // Cache for easy access
             const sellPriceInput = rowElement.querySelector('input[name="sell_price[]"]'); // Cache for easy access
                 ...
                 if (currentFilteredProducts.length > 0) {
                     currentFilteredProducts.forEach(product => {
                         ...
                         div.addEventListener('click', function() {
                             ...
                             // MODIFICATION START: Format prices on product selection
                             const costPriceNum = (product.cost_price !== undefined && product.cost_price !== null && product.cost_price !== '') ? parseFloat(product.cost_price) : null;
                             const sellPriceNum = (product.price !== undefined && product.price !== null && product.price !== '') ? parseFloat(product.price) : null;

                             if (purchasePriceInput) {
                                 purchasePriceInput.value = costPriceNum !== null ? costPriceNum.toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '';
                             }
                             if (sellPriceInput) {
                                 sellPriceInput.value = sellPriceNum !== null ? sellPriceNum.toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '';
                             }
                             // MODIFICATION END
         ...
         if(purchaseForm) {
             purchaseForm.addEventListener('submit', function(event) {
                 ...
                 for (let i = 0; i < itemRowsNodeList.length; i++) {
                    const row = itemRowsNodeList[i];
                    const productId = row.querySelector('.actual-product-id').value;
                    const quantity = row.querySelector('.quantity-input').value;
                    const purchasePrice = row.querySelector('.purchase-price-input').value;
                    const sellPrice = row.querySelector('.sell-price-input').value;

                    if (productId && quantity && purchasePrice && sellPrice) { // Basic check, can be more thorough
                        hasFilledItem = true;
                        // Validate numeric fields after unformatting
                        if (unformatRupiah(purchasePrice) <= 0 || unformatRupiah(sellPrice) < 0 || parseFloat(quantity) <= 0) {
                            formIsValid = false;
                            if (typeof Swal !== 'undefined') Swal.fire('Input Tidak Valid', `Data item pada baris ke-${i+1} tidak valid (Harga/Qty).`, 'warning'); 
                            else alert(`Data item pada baris ke-${i+1} tidak valid (Harga/Qty).`);
                            event.preventDefault(); return;
                        }
                    } else if (itemRowsNodeList.length === 1 && (!productId && !quantity && !purchasePrice && !sellPrice)) {
                        // If only one row and it's completely empty, it's an error
                         formIsValid = false;
                         if (typeof Swal !== 'undefined') Swal.fire('Input Tidak Lengkap', 'Harap isi detail item pembelian.', 'warning'); else alert('Harap isi detail item pembelian.');
                         event.preventDefault(); return;
                    } else if (productId || quantity || purchasePrice || sellPrice) { 
                        // If partially filled row
                        formIsValid = false;
                        if (typeof Swal !== 'undefined') Swal.fire('Input Tidak Lengkap', `Harap lengkapi data item pada baris ke-${i+1}.`, 'warning'); 
                        else alert(`Harap lengkapi data item pada baris ke-${i+1}.`);
                        event.preventDefault(); return;
                    }
                 }

                 if (!hasFilledItem && itemRowsNodeList.length > 0) { // No item has been properly filled
                    formIsValid = false;
                    if (typeof Swal !== 'undefined') Swal.fire('Input Tidak Lengkap', 'Tidak ada item pembelian yang valid.', 'warning'); else alert('Tidak ada item pembelian yang valid.');
                    event.preventDefault(); return;
                 }
                 
                 // If validation passes and form submits, PHP redirect will handle clearing localStorage.
                 // Note: The values for purchase_price[] and sell_price[] will be submitted as formatted strings (e.g., "100.000,00").
                 // The PHP backend needs to be able to parse these formatted strings correctly.
             });
         }
         ...
         function calculateDueDate() {
             if (fpPurchaseDate && fpPurchaseDate.selectedDates.length > 0 && dueDaysInput && dueDaysInput.value !== '') {
                 ...
                 if (!isNaN(days) && days >= 0) {
                     ...
                     if (fpDueDate) { fpDueDate.setDate(pDate, true); } 
                 }
             }
         }
         ...
         if (urlParams.has('purchase_success')) {
             ...
             loadPrimaryFormData(); 
             const itemsWereLoaded = loadItemRowsData(); 
